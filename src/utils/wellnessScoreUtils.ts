...
export const getScoreColor = (score: number): string => {
  if (score < 3.0) return "bg-red-100 text-red-600 border-red-200"; // Unhealthy
  if (score < 4.0) return "bg-yellow-100 text-yellow-600 border-yellow-200"; // Needs Attention
...
export const getRecommendation = (metricId: string, metricName: string, score: number): string => {
  if (score >= 4.7) {
    ...
    return `This area needs some attention. Focus on small daily improvements to your ${metricName.toLowerCase()}.`;
  } else {
    return `This area needs significant attention. Prioritize improving your ${metricName.toLowerCase()} with daily habits.`;
  }
};
