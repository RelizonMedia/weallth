...
const MetricHistoryChart = ({
  ...
  const chartData = data.map(entry => {
    ...
    return {
      ...
      category: score < 4.0 ? "Unhealthy" : score < 4.5 ? "Healthy" : score < 4.7 ? "Great" : "Amazing",
      dateObj: date // Add Date object for sorting
    };
  })
  // Sort data chronologically - oldest to newest
  .sort((a, b) => a.dateObj.getTime() - b.dateObj.getTime()).filter(item => item.score > 0); // Only include days with ratings
        ...
        <div className="h-[200px] w-full">
          <ResponsiveContainer width="99%" height="100%">
            ...
            bottom: 20
              ...
              fontSize: 10
            }} angle={-30} textAnchor="end" height={50} tickMargin={8} scale="point" />
              <YAxis tick={{
              fontSize: 10
            }} domain={[0, 5]} ticks={[0, 1, 2, 3, 4, 5]} width={25} />
              <Tooltip formatter={(value: number) => [`${value.toFixed(1)}`, `${metric.name} Score`]} labelFormatter={(_, data) => {
              ...
              <Area dataKey="score" y1={4.7} y2={5} fill="#8B5CF6" fillOpacity={0.2} strokeOpacity={0} name="Amazing" />
              <Area dataKey="score" y1={4.5} y2={4.7} fill="#0EA5E9" fillOpacity={0.2} strokeOpacity={0} name="Great" />
              <Area dataKey="score" y1={4.0} y2={4.5} fill="#4ECDC4" fillOpacity={0.2} strokeOpacity={0} name="Healthy" />
              <Area dataKey="score" y1={0} y2={4.0} fill="#F97316" fillOpacity={0.2} strokeOpacity={0} name="Unhealthy" />
              ...
              <Line type="monotone" dataKey="score" name={`${metric.name}`} stroke="#000" strokeWidth={2} dot={CustomDot} activeDot={CustomActiveDot} connectNulls />
            </LineChart>
          </ResponsiveContainer>
        </div>
